        ...
        {!loading && (
          <div className="space-y-2">
            {sortedEntries.map((entry, index) => (
              <div
                key={entry.uid}
                className={`flex items-center gap-4 p-4 rounded-xl border bg-gradient-to-r transition-all
                  ${getRankColor(index)}
                  ${entry.uid === user?.uid ? 'ring-1 ring-emerald-500/50' : ''}
                `}
              >
                {/* Rank */}
                <div className="w-12 text-center text-lg font-bold shrink-0">
                  {getRankBadge(index)}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-white font-semibold truncate">
                    {entry.displayName}
                    {entry.uid === user?.uid && (
                      <span className="text-emerald-400 text-xs ml-2">(você)</span>
                    )}
                  </p>
                  <p className="text-gray-500 text-xs">
                    {entry.gamesPlayed} partidas jogadas
                  </p>
                </div>
